~(function(win) {
	var htmls =
		'<input type="file" name="" id="file_input" class="imgFiles" style="display: none" accept="image/gif,image/jpeg,image/jpg,image/png,image/svg" multiple>' +
		// '<div class="header">' +
		// '    <span class="imgTitle">' +
		// '        精彩图库' +
		// '        <b>' +
	...
	ImgUploadeFiles.prototype = {
		...
		ready: function() {
			...
			// this.imgcontent.innerHTML = '请上传<b style="color:red">' + this.MAX + '</b>张' + _self.MW + ' * ' +
			// 	_self.MH + '像素的图片';
		...
		fileBtn: function(c, f) {
			...
			$(c).off().on('click', function() {
				...
				$(f).off().on('change', function() {
					var _this = this;
					_private.startUploadImg(_imgAll, _this.files, _self.MAX, _self.callback,
						_self.MW, _self.MH);
				});
			});
		}
	...
	var _private = {
		startUploadImg: function(o, files, MAX, callback, W, H) {
			...
			var imgTimeSlice = _this.timeChunk(fileImgArr, function(file) {
				...
				//调用图片类
				var up = new ImgFileupload(o,file, file.fileName, file.fileSrc, file.fileSize, callback);
				up.init();
			}, 1);
			imgTimeSlice(); //调用分时函数
		},
	...
	var ImgFileupload = function(b,file, imgName, imgSrc, imgSize, callback) {
		this.b = b;
		this.file = file;
